...
from lstm.utils.config import cfg,get_encode_decode_dict 
...
def write_image_annotation_pairs_to_tfrecord(img_path, tfrecords_filename):
    ...
    for root,subfolder,fileList in os.walk(img_path):
        for fname in fileList:
            ...
            code =fname.split('/')[2].split('.')[0].split('_')[1].split('-')
            #print(code)
            aligned_code = code[:]
            ...
            context = tf.train.Features(feature={
                ...
                'label_len': _int64_feature(len(code)), #tesnsorflow 自带 ctc不需要这个
...
def read_test(tfrecords_fiename=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        image_batch, label_batch, label_len_batch,time_step_batch = tf.train.shuffle_batch([image,label,label_len,time_step],
                                                                                            batch_size=2,
                                                                                            capacity=500,
                                                                                            num_threads=2,min_after_dequeue=100)
...
if __name__=='__main__':
    #wrtie_test(img_path='F:/tensorflowenv/tensorflow/ocr/train',tfrecords_filename='F:/tensorflowenv/tensorflow/ocr/ifrecordtrain/train_4_6.tfrecords')
    #wrtie_test(img_path='F:/tensorflowenv/tensorflow/ocr/test',tfrecords_filename='F:/tensorflowenv/tensorflow/ocr/ifrecordtest/val_4_6.tfrecords')
    read_test(tfrecords_fiename='F:/tensorflowenv/tensorflow/ocr/ifrecordtest/val_4_6.tfrecords')
    #print(1)